...
void scl_crc_decoder_decode_test()
{
    const size_t code_length = 8;
    const size_t info_length = 2;
    const std::vector<int> frozen_bits{ 1, 1, 0, 0, 1, 1, 0, 0 };
    ...
    pcstdc::SCLCRCDecoder decoder(decoder_params, tdc, frozen_bits, crc_handler, 4);
...
void scl_crc_decoder_test()
{
    scl_crc_decoder_decode_test();
}
